...
async function runMigration() {
  ...
  try {
    ...
    `;
    console.log("✓ Created generation_task table");

    try {
      await sql`
        ALTER TABLE "generation_task" 
        ADD CONSTRAINT "generation_task_user_id_user_id_fk" 
        FOREIGN KEY ("user_id") REFERENCES "public"."user"("id") 
        ON DELETE cascade ON UPDATE no action
      `;
      console.log("✓ Added generation_task foreign key");
    } catch (e: any) {
      if (e.code === "42710") {
        console.log("✓ Generation_task foreign key already exists");
      } else {
        throw e;
      }
    }
